...
fs.appendFileSync('release-dist/NOTES.md', '\n## Docker images\n\nNative AMD64/ARM64 Docker image archives include Node 22. No host Node upgrade or registry credential is needed. Verify SHA256SUMS, load the matching `*-docker-ARCH.tar.gz` with `docker load`, then pin the container to the `imageId` in its JSON manifest. Both legacy collector and multi-network console were smoke-tested as non-root/read-only containers. Private configuration is mounted at runtime. See deploy/DOCKER.md for cutover and rollback. These are Docker-save archives, not registry-published tags.\n');
